...
export function WalletContent() {
  ...
  const tabs: Array<{ id: WalletTab; label: string; icon: React.ComponentType<React.SVGProps<SVGSVGElement>> }> = [
    { id: "assets", label: t("tabs.assets"), icon: Coins },
    { id: "nfts", label: t("tabs.nfts"), icon: ImageIcon },
    { id: "transactions", label: t("tabs.transactions"), icon: ArrowUpRight },
  ];
  ...
  const explorerUrl = blockchainConfig.networks[blockchainConfig.network].explorerUrl;
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {connected && (
          <Button variant="ghost" size="sm" onClick={refreshBalance} disabled={isLoadingBalance} className="gap-2">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
          ...
          <div className="p-6 space-y-3">
            ...
            {supportedWallets.map((w) => {
              const isInstalled = installedWallets.some((iw) => iw.id === w.id);

              return (
                <button
                  key={w.id}
                  onClick={() => (isInstalled ? connect(w.id) : window.open(w.downloadUrl, "_blank"))}
                  disabled={connecting}
                  className={cn(
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <div className="text-xs text-gray-500">
                      {isInstalled ? t("walletInstalled") : t("walletNotInstalled")}
                    </div>
                  </div>
                  {isInstalled ? (
                    connecting ? (
            ...
            <div className="p-6 bg-linear-to-br from-primary-600 to-secondary-DEFAULT text-white">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-sm text-white/70">{walletName}</div>
                ...
                <Button variant="ghost" size="sm" onClick={disconnect} className="text-white/70 hover:text-white hover:bg-white/10">
                ...
                {isLoadingBalance ? (
                  ...
                  <>
                    <div className="text-4xl font-bold font-heading">
                      {balance?.adaFormatted || "0.00"} <span className="text-lg text-white/70">₳</span>
                    </div>
                ...
                <div className="text-center bg-white/10 rounded-xl p-3">
                  <div className="flex items-center justify-center gap-1">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                      ...
                      <text x="12" y="16" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#8B6914" fontFamily="sans-serif">S</text>
                    </svg>
              ...
              <motion.div key="assets" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
                ...
                <div className="flex items-center gap-4 p-4 bg-white rounded-xl border border-gray-100">
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-gray-900">Cardano</div>
                ...
                <div className="flex items-center gap-4 p-4 bg-white rounded-xl border border-gray-100">
                  <div className="w-10 h-10 rounded-full bg-amber-50 flex items-center justify-center">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                      <circle cx="12" cy="12" r="10" fill="#DAA520" />
                      <circle cx="12" cy="12" r="8" fill="#F5D060" />
                      <text x="12" y="16" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#8B6914" fontFamily="sans-serif">S</text>
                    </svg>
                  </div>
                  <div className="flex-1">
                    <div className="font-medium text-gray-900">SandjaCoin</div>
                    <div className="text-xs text-gray-500">SNDJ</div>
                  </div>
                  <div className="font-semibold text-gray-900">0 SNDJ</div>
                </div>

                {(!balance || balance.assets.length === 0) && (
                  <div className="text-center py-8 text-sm text-gray-400">{t("noAssets")}</div>
                )}
              </motion.div>
            )}
