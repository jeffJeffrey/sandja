...
import { getExplorerUrl, getIpfsUrl } from "@/services/cardano.service";
...
export function WalletContent() {
  ...
  const tabs: { id: WalletTab; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
  ...
  // Mock transactions (à remplacer par des vrais plus tard)
  const mockTransactions = [
    {
      id: "1",
      type: "received" as const,
      amount: "+50 SNDJ",
      description: t("transactions.quizReward"),
      date: "2025-02-08",
    },
    {
      id: "2",
      type: "sent" as const,
      amount: "-2 ADA",
      description: t("transactions.nftPurchase"),
      date: "2025-02-07",
    },
    {
      id: "3",
      type: "received" as const,
      amount: "+100 SNDJ",
      description: t("transactions.dailyReward"),
      date: "2025-02-06",
    },
  ];
    ...
    <div className="container mx-auto px-4 py-6 max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {connected && (
          ...
          >
            <RefreshCw
              className={cn("w-4 h-4", isLoadingBalance && "animate-spin")}
            />
            {tCommon("retry")}
          </Button>
        )}
          ...
          <div className="p-6 space-y-3">
            ...
            {supportedWallets.map((w) => {
              const isInstalled = installedWallets.some(
                (iw: any) => iw.id === w.id || iw.name.toLowerCase() === w.id
              );
            ...
            {activeTab === "assets" && (
              ...
              >
                ...
                {balance?.assets.map((asset) => (
                  ...
                  >
                    <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 text-xs font-mono">
                ...
                {balance && balance.nfts.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {balance.nfts.map((nft) => (
                      <div
                        key={nft.unit}
                        className="rounded-xl border border-gray-100 bg-white overflow-hidden group hover:shadow-md transition-shadow"
                      >
