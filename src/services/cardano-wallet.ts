...
/**
 * Get deep link for a wallet
 */
function getDeepLink(walletId: string): string {
  const map: Record<string, string> = {
    'yoroi': 'yoroi-wallet://mainnet/dapp?url=',
    'flint': 'flint://dapp?url=',
    'eternl': 'eternl://browser?url=',
    'begin': 'https://begin-wallet.app.link/browse?dappUrl=',
  };
  return map[walletId] || '';
}
...
export function getInstalledWallets(): CIP30WalletInfo[] {
  if (typeof window === "undefined" || (!window.cardano && !isMobile())) return [];
  const knownWallets = ["nami", "eternl", "flint", "yoroi", "lace", "typhon", "gerowallet"];
  const knownMobileWallets = ["eternl", "flint", "yoroi"];
  const wallets: CIP30WalletInfo[] = [];

  if (isMobile() && !window.cardano) {
    // On mobile, assume known mobile wallets might be installed
    knownMobileWallets.forEach((id) => {
      wallets.push({
        id,
...
export async function connectWallet(walletId: string): Promise<CIP30WalletAPI> {
  if (typeof window === "undefined" || !window.cardano) {
    if (isMobile()) {
      const deepLink = getDeepLink(walletId) + encodeURIComponent(window.location.href);
      if (deepLink) {
        window.location.href = deepLink;
        throw new Error(`Redirecting to ${walletId} app...`);
      }
    }
